	...
	public static void sellCommand(final ItemStack item, final Player player, final double price) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (price >= plugin.getLang().getDouble("Transaction.Sell.DontApply")) {

					Economy.TaxType taxType = Economy.TaxType.valueOf(plugin.getLang().getString("Transaction.Sell.Tax"));

					if (taxType != Economy.TaxType.NONE) {

						double taxAmount = taxType.calculateTax(price, plugin.getLang().getDouble("Transaction.Sell.Amount"));

						List<String> items = plugin.getLang().getStringList("Transaction.Sell.Items");
				...
				if (freeSlot < 0) {
					TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Sell.ShopFull"));
					return;
				}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (onlineInventories.containsKey(seller.getUniqueId())) {
					...
					if (data.equalsIgnoreCase("Not Set")) {
						TextUtil.sendMessage(buyer, plugin.getLang().getString("Commands.Shop.NotSet").replaceAll("%player%", seller.getName()));
					} else {
					...
					TextUtil.sendMessage(buyer, plugin.getLang().getString("Commands.Shop.NotFound").replaceAll("%player%", seller.getName()));
				}
		...
		new BukkitRunnable() {
			...
			public void run() {
				if (databaseHandler.containsPlayer(player.getUniqueId())) {
					...
					TextUtil.sendMessage(sender, plugin.getLang().getString("Commands.Delete.Removed").replaceAll("%player%", player.getName()));
				} else {
					TextUtil.sendMessage(sender, plugin.getLang().getString("Commands.Delete.NotFound").replaceAll("%player%", player.getName()));
				}
			}
		}.runTaskAsynchronously(plugin);
	}
	...
	public static void cleanDatabase(final CommandSender player, final int days) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Clean.Finished")
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (material == Material.AIR || material == null) {
					TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Search.InvalidMaterial"));
					return;
				}

				TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Search.Searching").replace("%material%", material.name()));

				Multimap<Double, OfflinePlayer> results = databaseHandler.search(material);

				if (results.size() == 0) {
					long time = System.currentTimeMillis() - start;
					TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Search.Nothing").replaceAll("%time%", String.valueOf(time)));
					return;
				}
				...
				for (Map.Entry<Double, Collection<OfflinePlayer>> entry : treeMap.entrySet()) {
					...
					for (OfflinePlayer offlinePlayer : value) {
						...
						if (spigot) {
							TextComponent text = new TextComponent(TextUtil.colorize(plugin.getLang().getString("Commands.Search.Results")
							...
							text.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(TextUtil.colorize(plugin.getLang().getString("Commands.Search.Hover").replaceAll("%player%", offlinePlayer.getName()))).create()));
							...
							format.add(plugin.getLang().getString("Commands.Search.Results")
									.replaceAll("%counter%", String.valueOf(counter))
									.replaceAll("%price%", NumberUtil.stringFormatDecimalPlaces(price))
									.replaceAll("%player%", offlinePlayer.getName())
									.replaceAll("%material%", material.name()));
						...
						if (counter > plugin.getLang().getInt("Settings.SearchSize")) break;
					}
				...
				TextUtil.sendMessage(player, plugin.getLang().getString("Commands.Search.Bottom")
	...
	public static void convertToMySQL(final CommandSender sender) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				Preconditions.checkNotNull(files, "There are no files.");
				...
				TextUtil.sendMessage(sender, plugin.getLang().getString("Commands.Convert.SuccessToMySQL")
						.replaceAll("%time%", String.valueOf(time))
	...
	public static void convertToFile(final CommandSender sender) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				plugin.getLang().set("Database.Type", "FILE");
				plugin.getLang().save();

				TextUtil.sendMessage(sender, plugin.getLang().getString("Commands.Convert.SuccessToFile")
						.replaceAll("%time%", String.valueOf(time))
